...
from shared.auth import TokenPayload, decode_jwt
...
def require_platform_role(*allowed_roles: str):
    ...
    Usage:
        @router.post("/van-ban")
        async def create(user = Depends(require_platform_role("BIEN_TAP", "QT_NOI_DUNG"))):
            ...
    """

    async def role_checker(current_user: CurrentUserDep) -> TokenPayload:
        # SUPER_ADMIN luon bypass kiem tra quyen
...
def require_lanh_dao():
    ...
    Chap nhan: is_lanh_dao=True HOAC SUPER_ADMIN.
    """
