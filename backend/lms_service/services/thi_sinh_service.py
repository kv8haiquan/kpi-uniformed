...
class ThiSinhService:
    ...
    # ================================================================
    # RESET LUOT THI (QT_DAO_TAO)
    # ================================================================

    def _snapshot_thi_sinh_raw(self, ts: ThiSinh) -> dict:
        """Chup NGUYEN TRANG ban ghi thi_sinh -> dict JSON-safe.

        Khac _snapshot_lan_thi (chi lay 1 lan thi de dua vao lich_su_thi): ham
        nay lay tat ca cot, ke ca cac cot se bi xoa trang, de con duong lui
        neu reset nham. Datetime/UUID/Decimal duoc doi sang kieu JSON hieu duoc.
        """
        snapshot: dict = {}
        for col in ThiSinh.__table__.columns:
            val = getattr(ts, col.key, None)
    ...
    async def reset_luot_thi(
        ...
        """Reset bai thi cua 1 thi sinh — chi QT_DAO_TAO/SUPER_ADMIN.

        Dung cho tinh huong nguoi khac dang nhap nham tai khoan va lam bai,
        hoac su co ky thuat giua chung. Truoc khi sua, toan bo ban ghi (ke ca
        bai lam va cac lan vi pham) duoc chup vao lms.lich_su_reset_thi de
        con duong lui — khong bao gio xoa trang ma khong luu vet.

        XOA_SACH     : ve CHUA_THI, xoa diem + bai lam + lich su, tra lai du luot.
        MO_KHOA_LUOT : giu ket qua cu, chi go co da_xac_nhan de thi tiep luot sau.
        """
        if loai_reset not in ("XOA_SACH", "MO_KHOA_LUOT"):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={"success": False, "error": {"code": "DGNL_050", "message": "Loại reset không hợp lệ"}},
            )
        ...
        # Chup nguyen trang TRUOC khi sua bat cu thu gi
        snapshot = self._snapshot_thi_sinh_raw(ts)
        vp_rows = (await self.db.execute(
            select(ViPhamThi).where(ViPhamThi.thi_sinh_id == ts.id)
        )).scalars().all()
        snapshot["_vi_pham"] = [
            {
                "lan_thi": v.lan_thi,
                "loai_vi_pham": v.loai_vi_pham,
                "thoi_gian": v.thoi_gian.isoformat() if v.thoi_gian else None,
                "ly_do": v.ly_do,
            }
            for v in vp_rows
        ]
        trang_thai_truoc = ts.trang_thai
        lan_thi_truoc = ts.lan_thi_hien_tai
        diem_truoc = ts.diem_tong

        if loai_reset == "MO_KHOA_LUOT":
            if ts.trang_thai != "DA_NOP":
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail={"success": False, "error": {"code": "DGNL_051", "message": "Chỉ mở khóa được thí sinh đã nộp bài"}},
                )
            if not ts.da_xac_nhan:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail={"success": False, "error": {"code": "DGNL_052", "message": "Thí sinh chưa bị khóa — vẫn còn quyền thi lại, không cần mở khóa"}},
                )
            if (ts.lan_thi_hien_tai or 0) >= (kt.so_lan_thi_toi_da or 1):
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail={"success": False, "error": {"code": "DGNL_053", "message": f"Thí sinh đã dùng hết {kt.so_lan_thi_toi_da} lượt — mở khóa không có tác dụng, hãy dùng 'Xóa sạch' nếu muốn cho thi lại từ đầu"}},
                )
            # Chi go khoa, giu nguyen diem va lich su cac lan da thi
            ts.da_xac_nhan = False
            ts.thoi_gian_xac_nhan = None
        else:
            # XOA_SACH — ve dung hinh dang mot ban ghi CHUA_THI
            ts.trang_thai = "CHUA_THI"
            ts.lan_thi_hien_tai = 0
            ts.diem_tong = None
            ts.xep_loai = None
            ts.so_cau_dung = None
            ts.so_cau_sai = None
            ...
            ts.da_xac_nhan = False
            ts.thoi_gian_xac_nhan = None
            ts.thoi_gian_bat_dau = None
            ts.thoi_gian_nop = None
            ts.thoi_gian_lam_giay = None
            # Vi pham gan voi lan thi da bi xoa -> xoa theo (da nam trong snapshot)
            for v in vp_rows:
                await self.db.delete(v)

        ts.updated_at = now_vn()

        # Go phien thi cu: thiet bi cua nguoi lam nham con giu token, neu de lai
        # ho co the nop tiep vao ban ghi vua reset.
        phien = (await self.db.execute(
            select(PhienThi).where(PhienThi.thi_sinh_id == ts.id)
        )).scalars().all()
        for p in phien:
            await self.db.delete(p)

        self.db.add(LichSuResetThi(
            thi_sinh_id=ts.id,
            ky_thi_id=ky_thi_id,
            ky_thi_id_snapshot=ky_thi_id,
            cong_chuc_id=cong_chuc_id,
            nguoi_reset_id=uuid.UUID(str(user.sub)),
            loai_reset=loai_reset,
            ly_do=ly_do,
            trang_thai_truoc=trang_thai_truoc,
            lan_thi_truoc=lan_thi_truoc,
            diem_truoc=diem_truoc,
            du_lieu_truoc=snapshot,
            thoi_gian=now_vn(),
        ))
        await self.db.commit()
        ...
        return {
            "thi_sinh_id": str(ts.id),
            "loai_reset": loai_reset,
            "trang_thai_truoc": trang_thai_truoc,
            "trang_thai_sau": ts.trang_thai,
            "lan_thi_truoc": lan_thi_truoc,
            "lan_thi_sau": ts.lan_thi_hien_tai,
            "diem_truoc": float(diem_truoc) if diem_truoc is not None else None,
            "so_lan_thi_toi_da": kt.so_lan_thi_toi_da or 1,
        }

    async def danh_sach_lich_su_reset(
        self,
        ky_thi_id: uuid.UUID,
        user: TokenPayload,
        cong_chuc_id: Optional[uuid.UUID] = None,
    ) -> list[dict]:
        """Nhat ky reset cua 1 ky thi (loc theo 1 thi sinh neu truyen cong_chuc_id)."""
        cc = CongChucRef.__table__.alias("cc")
        nr = CongChucRef.__table__.alias("nr")
        ...
        stmt = (
            select(
                ...
                cc.c.ho_ten,
                cc.c.ma_cc,
                nr.c.ho_ten.label("nguoi_reset_ten"),
                nr.c.ma_cc.label("nguoi_reset_ma_cc"),
            )
            .outerjoin(cc, LichSuResetThi.cong_chuc_id == cc.c.id)
            .outerjoin(nr, LichSuResetThi.nguoi_reset_id == nr.c.id)
            .where(LichSuResetThi.ky_thi_id_snapshot == ky_thi_id)
            .order_by(LichSuResetThi.thoi_gian.desc())
        )
        if cong_chuc_id:
            stmt = stmt.where(LichSuResetThi.cong_chuc_id == cong_chuc_id)

        rows = (await self.db.execute(stmt)).all()
        return [
            {
                "id": log.id,
                "ky_thi_id": log.ky_thi_id,
                "cong_chuc_id": log.cong_chuc_id,
                "nguoi_reset_id": log.nguoi_reset_id,
                "loai_reset": log.loai_reset,
                "ly_do": log.ly_do,
                "trang_thai_truoc": log.trang_thai_truoc,
                "lan_thi_truoc": log.lan_thi_truoc,
                "diem_truoc": log.diem_truoc,
                "thoi_gian": log.thoi_gian,
                "ho_ten": ho_ten,
                "ma_cc": ma_cc,
                "nguoi_reset_ten": nguoi_ten,
                "nguoi_reset_ma_cc": nguoi_ma,
            }
            for log, ho_ten, ma_cc, nguoi_ten, nguoi_ma in rows
        ]
