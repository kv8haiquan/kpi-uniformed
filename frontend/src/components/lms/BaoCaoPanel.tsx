/**
 * src/components/lms/BaoCaoPanel.tsx
 * =====================================
 * Panel báo cáo quản lý đào tạo — 3 sub-tabs:
 *   1. Báo cáo đơn vị   — GET /bao-cao/don-vi/{id}?thang=&nam=
 *   2. Báo cáo khóa học — GET /bao-cao/khoa-hoc/{id}
 *   3. Thống kê khảo sát — GET /khoa-hoc/{id}/khao-sat/thong-ke
 */
...
type BaoCaoTab = 'don-vi' | 'khoa-hoc' | 'khao-sat';
...
export default function BaoCaoPanel() {
  const [tab, setTab] = useState<BaoCaoTab>('don-vi');
  ...
  const loadBcKH = async () => {
    if (!selKH) return;
    setLoadingKH(true);
    setErrKH('');
    setBcKH(null);
    try {
      const r = await baoCaoApi.khoaHoc(selKH);
      setBcKH(r.data.data ?? r.data);
    } catch (err: any) {
      setErrKH(err?.response?.data?.detail?.error?.message || 'Không thể tải báo cáo');
    } finally {
      setLoadingKH(false);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="flex gap-1 mb-6 bg-gray-100 rounded-lg p-1">
        {([
          ['don-vi',   '🏢 Báo cáo đơn vị'],
          ['khoa-hoc', '📚 Báo cáo khóa học'],
          ['khao-sat', '⭐ Thống kê khảo sát'],
        ] as [BaoCaoTab, string][]).map(([k, label]) => (
      ...
      {/* ── Tab: Báo cáo khóa học ───────────────────────────── */}
      {tab === 'khoa-hoc' && (
        <div className="space-y-4">
          <div className="flex gap-3">
            <select value={selKH} onChange={(e) => setSelKH(e.target.value)}
            ...
            <button onClick={loadBcKH} disabled={!selKH || loadingKH}
              className="px-5 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium disabled:opacity-50 hover:bg-blue-700">
              {loadingKH ? 'Đang tải...' : 'Xem'}
            </button>
          </div>

          {errKH && <p className="text-sm text-red-600">{errKH}</p>}

          {bcKH && (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {[
                { label: 'Tổng học viên', value: bcKH.tong_hoc_vien ?? bcKH.so_hoc_vien },
                { label: 'Đang học',      value: bcKH.dang_hoc },
                { label: 'Hoàn thành',    value: bcKH.hoan_thanh },
                { label: 'Tỷ lệ HT',     value: bcKH.ty_le_hoan_thanh != null ? `${bcKH.ty_le_hoan_thanh}%` : '—' },
                { label: 'Điểm TB',       value: bcKH.diem_tb_chung != null ? `${bcKH.diem_tb_chung}%` : '—' },
                { label: 'Chứng chỉ',    value: bcKH.so_chung_chi },
              ].map((s, i) => (
                <div key={i} className="bg-white border border-gray-200 rounded-lg p-4 text-center">
